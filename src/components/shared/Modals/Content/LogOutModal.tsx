...
function LogOutModal(prop: { cancelModal: () => void }) {
    ...
    return (
        <div className=" m-10 md:w-96 h-auto bg-white p-10 rounded-xl">
            ...
            <button className='md:text-xl py-5 px-5 bg-accentColor w-30 rounded-xl text-white' onClick={prop.cancelModal}>Cancel</button>

            <button className='md:text-xl py-5 px-4 bg-secondaryColor w-30 rounded-xl float-right' onClick={onProceed}>Proceed</button>
        </div>
    );
}
